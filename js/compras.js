...
function carregarItensCliente(clienteId) {
    ...
    // Buscar os itens do cliente
    window.dbRef.projetos.child(clienteId).once('value')
        .then((snapshot) => {
            console.log('Resposta do Firebase recebida para itens');
            ...
            // Verifica se o projeto existe e tem itens
            if (!projeto || !projeto.itens || objetoVazio(projeto.itens)) {
                console.log('Nenhum item encontrado para o cliente');
                nenhumItem.classList.remove('d-none');
                inicializacaoEmProgresso = false;
                return;
            }
            
            console.log('Itens encontrados para o cliente');
            nenhumItem.classList.add('d-none');
            
            // Armazenar todos os itens para referência
            todosItens = projeto.itens;
            
            // Preparar dados para DataTables
            let dataSet = [];
            
            // Iterando sobre as chaves do objeto itens
            Object.keys(projeto.itens).forEach(itemId => {
                const item = projeto.itens[itemId];
                
                // Adicionar o ID ao item para referência futura
                item.id = itemId;
                
                // Calcular a necessidade (quantidade - quantidadeComprada)
                ...
                } else if (item.quantidadeComprada > 0) {
                    status = 'Parcial';
                    statusClass = 'text-warning';
                }
                
                // Preparar o botão de toggle para detalhes
                const btnToggle = `
                    <button class="btn btn-sm btn-outline-secondary btn-toggle-item" data-id="${itemId}">
                        <i class="fas fa-info-circle"></i>
                    </button>
                `;
                
                // Preparar o checkbox para seleção
                const checkbox = `
                    <div class="form-check">
                        <input class="form-check-input checkbox-item" type="checkbox" data-id="${itemId}" onchange="atualizarSelecao()">
                    </div>
                `;
                
                // Preparar os botões de ação
                let acoes = '';
                
                // Botão de editar prazo (apenas para itens com compra)
                if (item.quantidadeComprada > 0 && item.prazoEntrega) {
                    acoes += `
                        <button type="button" class="btn btn-sm btn-warning me-1" onclick="editarPrazoEntrega('${itemId}')">
                ...
                dataSet.push([
                    ...
                    item.quantidade || '0',
                    item.lista || 'N/A',
                    necessidade > 0 ? necessidade : '0',
                    item.quantidadeComprada || '0',
                    `<span class="${statusClass}">${status}</span>${acoes}`
                ]);
            });
            
            // CORREÇÃO: Destruir a instância existente do DataTable se houver
 ...
 * @param {Array} itensIds - IDs dos itens selecionados
 * @param {string} clienteId - ID do cliente
...
function processarCompra(itensIds, clienteId, fornecedor, prazoEntrega, quantidadePersonalizada = null) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Converter o prazo para timestamp
        const prazoTimestamp = new Date(prazoEntrega).getTime();
        ...
        projetoRef.once('value')
            .then((snapshot) => {
                ...
                if (!projeto || !projeto.itens) {
                    reject('Projeto ou itens não encontrados');
                    return;
                }
                ...
                itensIds.forEach(itemId => {
                    ...
                    if (!item) {
                        console.warn(`Item ${itemId} não encontrado no projeto`);
                        return;
                    }
                    ...
                    if (quantidadePersonalizada !== null) {
                        ...
                        const necessidadeTotal = itensIds.reduce((total, id) => {
                            const itemAtual = projeto.itens[id];
                            if (itemAtual) {
                                const necessidadeItem = parseInt(itemAtual.quantidade || 0) - parseInt(itemAtual.quantidadeComprada || 0);
                                return total + (necessidadeItem > 0 ? necessidadeItem : 0);
                            }
                            return total;
                        }, 0);
                        
                        const necessidadeItem = parseInt(item.quantidade || 0) - parseInt(item.quantidadeComprada || 0);
                    ...
                    if (quantidadeComprar <= 0) {
                        console.warn(`Item ${itemId} não tem quantidade a comprar`);
                        return;
                    }
                    ...
                    // Registrar a compra
                    updates[`itens/${itemId}/quantidadeComprada`] = novaQuantidadeComprada;
                    updates[`itens/${itemId}/fornecedor`] = fornecedor;
                    updates[`itens/${itemId}/prazoEntrega`] = prazoTimestamp;
                    updates[`itens/${itemId}/dataCompra`] = Date.now();
                    
                    console.log(`Item ${itemId}: Comprado ${quantidadeComprar} unidades (total: ${novaQuantidadeComprada})`);
                });
...
function atualizarPrazoEntrega(itemId, clienteId, novoPrazo) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Converter o prazo para timestamp
        const prazoTimestamp = new Date(novoPrazo).getTime();
        ...
        // Referência ao item
        const itemRef = window.dbRef.projetos.child(clienteId).child('itens').child(itemId);
        
        // Atualizar o prazo
        itemRef.update({
            prazoEntrega: prazoTimestamp
        })
            .then(() => {
...
function finalizarCompras(clienteId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        clienteRef.update({
            ...
            dataFinalizacaoCompras: Date.now()
        })
            .then(() => {
